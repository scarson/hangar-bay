...
*   [ ] **Implement Ship Contract List Endpoint (e.g., `/api/v1/contracts/ships`):**
    ...
    *   **Error Handling:** Use standard FastAPI `HTTPException` for errors (e.g., 404 if a related `type_id` for `ship_name` is not found in `EsiTypeCache`, though this should ideally be guaranteed by the aggregation service).
