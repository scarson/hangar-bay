...
*   [x] **Step 7: Update Integration Tests**
    *   Refactored the `TestClient` integration tests to resolve a critical routing issue (`/api/v1/contracts` vs. `/contracts`) and stabilize the test fixture pattern.
    *   Added comprehensive new test cases in `test_contract_filters.py` that verify the functionality of all advanced filter combinations (text search, price ranges, type IDs) and sorting options (by price, by name, ascending/descending).
...
- [x] A new [`services/contract_service.py`](/app/backend/src/fastapi_app/services/contract_service.py) module exists and contains the advanced query logic.
- [x] The [`ContractFilters`](/app/backend/src/fastapi_app/schemas/contracts.py) and `PaginatedResponse` Pydantic models are defined and used by the API.
- [x] The [`/contracts/`](/app/backend/src/fastapi_app/api/contracts.py) endpoint is updated to delegate its logic to the new service, simplifying the endpoint to a one-liner.
- [x] Necessary database indexes have been added via an [Alembic migration](/app/backend/src/alembic/versions/c199a09ccc55_add_contract_item_and_market_group_.py) to support efficient filtering on key fields (`is_ship_contract`, `start_location_name`, `title`, `type`, `status`).
- [x] [Integration tests](/app/backend/src/fastapi_app/tests/api/test_contract_filters.py) are updated and passing, covering the new filtering and sorting functionality.
...
### 7.1. Security
*   [x] **Secure Design:** The primary security design consideration is preventing SQL injection through the dynamic sorting parameter. This will be mitigated by mapping API-facing sort keys to specific, safe SQLAlchemy column objects.
*   [x] **Input Validation:** All incoming filter, sort, and pagination parameters will be strictly validated by the `ContractFilters` Pydantic model, using FastAPI's `Query` for constraints on types, ranges, and lengths.
...
*   [x] **Authentication/Authorization:** This is a public, unauthenticated endpoint. No specific authentication or authorization checks are required for this task.
*   [x] **Secrets Management:** N/A. This task does not involve handling any secrets, API keys, or credentials.
*   [x] **Dependency Management:** All dependencies are managed via `pdm` and pinned in `pyproject.toml`. No new dependencies are anticipated for this task, but if any were added, they would be subject to review.
*   **Notes:** The core security focus is robust input validation via Pydantic and safe handling of the dynamic `sort_by` parameter to prevent misuse. All items verified.
...
*   [ ] **Metrics:** No custom metrics were added. This remains an open item. We will consider adding a metric for query execution time to monitor the performance of the dynamic query generation.
*   **Notes:** Observability implementation for the search API has been moved to a dedicated task: [05.2-observability-for-search-api.md](./05.2-observability-for-search-api.md). This includes structured logging, metrics, and testing strategies. The comprehensive observability design guide was created at `/design/fastapi/guides/02-observability-guide.md` and the local Docker observability stack (Prometheus + Grafana) is provisioned and running.

### 7.3. Testing
*   [x] **Unit Tests:** Unit tests exist for the `contract_service` in `tests/services/test_contract_service.py`, verifying query logic in isolation.
*   [x] **Integration Tests:** `tests/api/test_contract_filters.py` provides comprehensive `TestClient` integration tests covering all filter types, sorting, and pagination.
*   [x] **Test Coverage:** High test coverage was achieved through the combination of service-level unit tests and API-level integration tests.
*   [x] **Test Data Management:** Tests use `conftest.py` fixtures to populate the test database with specific records designed to validate all filtering scenarios.
*   **Notes:** A robust set of integration and unit tests was successfully implemented, ensuring the complex filtering logic is behaving as expected.
...
*   [x] **Character Encoding:** The service operates assuming UTF-8 encoding for all text data, which is standard for FastAPI and modern web applications.
