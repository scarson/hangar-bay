...
*   [x] Initialized a virtual environment: `python -m venv .venv` within `backend/`. Added `.venv` to root `.gitignore`.
*   [x] Created an initial `backend/requirements.txt` with core dependencies (all version pinned):
        *   `fastapi==0.115.12`
        *   `uvicorn[standard]==0.34.3`
        *   `pydantic-settings==2.9.1` (for typed settings management)
        *   `python-dotenv==1.1.0` (for loading .env files)
        *   `SQLAlchemy==2.0.41` (ORM for database interaction)
        *   `alembic==1.16.1` (for database migrations)
        *   `asyncpg==0.30.0` (async PostgreSQL driver)
        *   `redis==6.2.0` (for Valkey/Redis cache interaction)
        *   `aiosqlite==0.21.0` (async SQLite driver, e.g., for local testing or simple DBs)
*   [x] Set up Flake8 for linting: `pip install flake8` and created a `.flake8` configuration file in `backend/`.
...
*   [x] Set up Black for formatting: `pip install black`. Configured to run from project root or `backend/`.
    *   **AI Prompt:** "Show command to format backend Python code using Black with default settings from the `backend/` directory or project root."
