...
*   [ ] **Step 1.1:** Define frontend data models in `src/app/features/contracts/contract.models.ts`. These TypeScript interfaces must precisely match the Pydantic schemas found in the backend's `schemas/contracts.py`.
...
*   [ ] **Step 1.2:** Define a comprehensive, immutable filter interface (`ContractSearchFilters`) in the same models file. This interface must align with the query parameters defined in the backend's `api/contracts.py` endpoint (`list_public_contracts`).
    *   `page: number`
    *   `size: number`
    *   `search?: string`
