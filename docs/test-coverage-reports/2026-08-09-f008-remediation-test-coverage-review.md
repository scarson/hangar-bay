...
- **backend-write N-11's `"a contract"` fallback** — unreachable behind the matcher's type gate, so
  ...
  unit — there is no other route — and `_SHIP_TYPE_LABELS[contract_type]` in place of the `.get`
  now fails that test while both label-rendering tests stay green.
