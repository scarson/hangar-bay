...
**[PR #98](https://github.com/scarson/hangar-bay/pull/98)** — `claude/contract-filter-bugfixes`. Classified `Review — serialization contract / schema changes`, all four checks green, **deliberately not merged**. Fixes four confirmed defects: the `is_bpc=false` filter (matched zero rows), courier contracts rendering as "Exchange", a courier serialization 500 (one null `start_location_id` fails an entire 50-row page, not just its row), and `collateral` being filterable/sortable but absent from every response. Adds pitfalls **ESI-3, FASTAPI-3, SQLA-3, TEST-18, TEST-19**.

Those IDs were renumbered late in the session. A parallel agent's PR #101 merged first and had independently claimed `ESI-2` and `TEST-17` for entirely different traps, and both agents had written a separate entry for the same date-bomb incident. PR #98 was rebased: its `ESI-2` became `ESI-3`, its `TEST-17` became `TEST-18`, its duplicate date-bomb entry was deleted in favour of the merged one, and the `# (pitfall ESI-2)` comment in `contract_service.py` was repointed to `ESI-3`. Verified after the rebase. The general rule this produced now lives in `docs/git-strategy.md` §Shared sequential identifiers.
...
- **69 of 70 regions in the frontend filter can never match** — we ingest The Forge only. Verified against production 2026-08-01: The Forge returns 33,941 contracts, Domain / Sinq Laison / Heimatar return 0 each. Same defect class as the "silent filter no-op" the analysis proposes making a product invariant, and it's live in the shipped UI. Left alone because the fix is a design choice (hide uncovered regions, label them, or show an explanatory empty state) rather than a bug with one right answer — see gap analysis §4.3.
- **The list response returns two dead fields and withholds a useful one.** `GET /contracts/` returns `status` (always `"unknown"`) and `date_completed` (always null), but not `start_location_region_id` — so the client can filter by region and can never show which region a row is in. Also prod-verified 2026-08-01.
