...
- `db.py` creates the engine with no pool tuning (`pool_pre_ping`, sizing) — a gap behind any pooler and across managed-PG restarts.
...
| `hangar-bay-cache` | Key Value (Valkey 8) | Free 25 MB (upgrade $10 if evicting) | Cache + sessions + ingestion lock + APScheduler jobstore; persistence off |
...
- **Internal wiring**: `DATABASE_URL`/`CACHE_URL` via blueprint `fromDatabase`/`fromService` references — never typed by a human (I-5).
...
**Two-stage sequencing around M3 (the collision map is the constraint):**

- **Stage 1 — scaffolding (collision-free, can start immediately):** touches only `src/alembic/`, `alembic.ini`, `check_alembic_version.py` (delete), `pyproject.toml` (add `pdm run` migration scripts). Work: delete stale revisions; restore `env.py`'s CLI invocation tail; enable `compare_type` + `compare_server_default`; wire async engine handling (`run_sync` against the asyncpg URL — no second driver dependency); generate + hand-review the baseline revision from current dev models. M3 touches none of these files (`docs/audits/m4-recon/data-migrations.md` §collision-map).
- **Stage 2 — cutover (AFTER the M3 merge, then regenerate the baseline):** M3 adds three tables, one `users` column, and the project's first partial unique index — the baseline must be re-autogenned (or a second revision stacked) after it lands, with **hand review of the partial index**: autogen renders `postgresql_where` imperfectly, and the predicate must match the model exactly or the matcher's `ON CONFLICT` dedup raises at runtime (SQLA-2 in session memory; M3 is adding the pitfall entry). Then: `main.py` lifespan keeps the dev-only `create_db_tables` path but production schema comes exclusively from `alembic upgrade head` run as **Render's pre-deploy command** (runs after build, before the old instance stops — with recreate deploys the old instance briefly serves on the new schema, so migrations must stay additive-compatible one release back; recorded as the operating rule). `conftest.py` stays on create_all for test speed — tests verify *models*; a dedicated test asserts baseline-migration ⇒ `Base.metadata` equivalence (offline `alembic-autogen-check` shape) so drift can't silently accumulate.
...
**Also in Stage 2:** `db.py` engine gains `pool_pre_ping=True` and explicit modest pool sizing (Render Basic's connection budget is small; exact numbers in the plan).
...
| `DATABASE_URL` / `CACHE_URL` | blueprint `fromDatabase`/`fromService` refs | never human-typed |
| `DATABASE_URL_TESTS` / `CACHE_URL_TESTS` | unset | test-only |

**Provisioning paths (I-5):** secret values (`ESI_CLIENT_SECRET`, `TOKEN_CIPHER_KEYS`) enter via the Render dashboard or blueprint `sync: false` declarations (names in git, values dashboard-held); connection strings via blueprint references; the CD credential lives only in GitHub Actions secrets. **Forbidden forms, named:** any `render`/`curl` CLI invocation with a secret in argv; secrets in `render.yaml` literals; secrets in chat or committed `.env` files. `.env.example` gains a commented production section documenting this inventory.

**Boot-time misconfig surfacing:** today a prod deploy with empty SSO config just 503s at request time (the startup warning is dev-only). M4 extends `warn_if_sso_unconfigured` to log the warning in production too (never crash — SSO-less operation stays valid; the marketplace works anonymously). Post-M3 change (`main.py`).
...
- **Trigger:** `push` to `main` (the release branch — deploys are publications, matching git-strategy §Release branch) + `workflow_dispatch` for manual/rollback deploys. `concurrency: group: deploy-production, cancel-in-progress: false` (queue, never cancel a mid-flight deploy; distinct from CI's `ci-${{ github.ref }}` group).
- **Permissions:** `contents: read` only; the Render deploy hook URLs are the credentials, stored as GitHub Actions secrets.
- **Jobs:** (1) `verify` — re-run the fast gates (backend pytest, frontend lint+typecheck+unit) on the release commit, belt+braces since dev→main PRs already ran CI; (2) `deploy` — POST the backend deploy hook, then the static-site deploy hook; Render builds both from the pinned commit (auto-deploy OFF on both services — CI is the sole trigger; **deploy hooks chosen over an account API key for least privilege**: each hook is scoped to one service); (3) `smoke` — poll `/ready` until healthy, then run the existing Playwright **live-smoke** lane (`E2E_LIVE=1`, `--project=live-smoke`) against the prod origin. Failure marks the run red and notifies; rollback is `workflow_dispatch` from the previous good SHA (Render redeploys that commit).
...
The aggregation service records `last_success_at` + outcome per run (Valkey key + Prometheus gauge `hangar_bay_last_ingest_success_timestamp`; Valkey chosen over a DB table — loss-tolerable operational signal, no migration, no write-amplification on user data). Read back by `/ready` (age field) and `/metrics`. This is the prerequisite the parked items key off.
...
1. **Prod EVE app registration** (Sam, developers.eveonline.com): new application, callback `https://<domain>/api/v1/auth/sso/callback`, **zero scopes** (M2 ships identity-only login; scope expansion is M3's recorded thread), yielding the prod `ESI_CLIENT_ID`/`ESI_CLIENT_SECRET`. Requires the domain decision first. Dev registration stays untouched.
...
| Phase | Content | Gate |
|---|---|---|
| 0 | Render verification spike (§3.5) — throwaway free project, no billing | none (free) |
| 1 | Collision-free implementation: Dockerfile, `render.yaml`, Caddyfile-fallback (if spike says so), Alembic Stage 1 scaffolding, `deploy.yml`, OpenAPI-drift CI job | none |
| 2 | Platform provisioning: accounts, billing, domain, DNS, secrets, prod EVE registration | **Sam** (billing + domain + EVE portal) |
| 3 | Post-M3 backend work: Alembic Stage 2 cutover + baseline regen, `/ready`, freshness surface, `/metrics` gate, `/cache-test` deletion, pool tuning, prod SSO warning | **M3 merged to dev** |
