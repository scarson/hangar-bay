...
1. **What should the default preference be?** Recommendation is `secure`, supported by both collateral-risk reasoning and the security-segmented ISK/jump pricing the freight services use (section 4). The supporting evidence for the convention is a web-search summary rather than a primary rate card, so it is worth one confirming look at a live freight service's rate page before F008 ships. **This is a one-line default, easily changed, and does not block the verdict.** Adam4EVE is already known from the contract-coverage gap analysis to pair a jumps column with a route-security tooltip, which independently supports "show the preference, don't hide it."
