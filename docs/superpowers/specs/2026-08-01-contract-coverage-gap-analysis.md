...
Two columns we persist do **not exist on the public items route** at all (they appear only on the authenticated character/corporation contract routes), so they can only ever be NULL *under public ingestion*: `raw_quantity` (empirically confirmed absent across 1,658 sampled live rows) and `is_singleton` (same class; spec-confirmed absent). The `min_runs`/`max_runs` filter being wired to `raw_quantity` is why it matches nothing — see §4.4.

**Keep them, don't drop them (decided 2026-08-01).** Dropping these two as dead weight was approved and then reversed on new information: Sam expects **character/corp contract ingestion to become important** (a player flagged it as useful). Checked against the spec, all four of our permanently-NULL columns are exactly what those routes return — `/characters/{id}/contracts` and `/corporations/{id}/contracts` carry `status` and `date_completed`; their `/items` sub-routes carry `raw_quantity` and `is_singleton`. So the columns are dead only under *this* pipeline and populate the moment that direction is taken. Dropping them buys nothing (nullable, unindexed) and costs two migrations to undo — the "keeping options open isn't YAGNI" case, and consistent with M5 Workstream D, which kept `status` on identical reasoning. They are documented at the model instead, so nobody wires a filter to them again.

**Starting inventory if that direction is taken:** those routes additionally return `acceptor_id`, `assignee_id`, `availability`, and `date_accepted`, for which we have no columns at all; and the authenticated bids routes return **`bidder_id`**, which the public bids route omits — so a user's own auctions could show who bid, where public auctions never can. The material new constraint is that this needs per-user ESI tokens, raising token-lifecycle and privacy questions the public pipeline does not have.
