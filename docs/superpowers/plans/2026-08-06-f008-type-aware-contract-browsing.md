...
Both fixes mutation-verified (4 more mutations, 4 killed; 18 in total across the review rounds). The lesson from D13's reversal is recorded in that entry: "describes the corpus rather than the rows" is not an exemption from WEB-1, because any value the row rendering consumes is about the rows at the moment it is consumed.
