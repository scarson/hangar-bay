...
- ~~**`FilterRail.hasActiveFilters` does not yet know about the item-level params.**~~ **CLOSED by Task D2**: the predicate now calls `hasOfferedItemFilters(search)`, which covers all nine offered-item params in one place, so a deep link carrying only a taxonomy or blueprint filter offers Clear filters. Pinned by `offers Clear filters for a taxonomy selection that arrived by URL`.
- **A window refocus that spans a readiness change costs one extra corpus-scale list request** (codex round-4 P2, accepted rather than fixed). On refocus React Query refetches both stale queries; the list starts under the current key, then the taxonomy answer flips the key and starts a second. It needs a readiness change while the tab was unfocused — i.e. an `ENRICHMENT_VERSION` resweep, once per release — and both requests are honest, so this is cost, not incorrectness. Not fixed because every fix is worse: gating the list on `!taxonomy.isFetching` would stall it behind the ordinary five-minute poll, and turning off `refetchOnWindowFocus` changes refresh semantics for every user to save one request for a few. Recorded because corpus-scale requests are exactly what the 2026-08-02 perf audit is about — if the resweep window ever coincides with load, this is where to look. Same-coverage polls, ordinary refocuses, and a taxonomy error after a success do NOT churn the key (verified in that review).
- **Criterion 1.8's lift is not applied to the offered-item filters, so an item-less segment's count reads 0 whenever one is active.** `_segment_counts` (`services/contract_service.py`) lifts `ships_only` for the item-less types precisely because nothing item-less can satisfy it; `category_id`, `group_id`, the three blueprint ranges and `is_bpc` are unsatisfiable by an item-less contract for the identical reason, and are not lifted. The client shows the served zero as-is — the numeral suppression that Task D2 first shipped was **withdrawn** in the codex review (finding 4), because the parser change it was covering for was itself withdrawn. So the zero is honest: clicking the segment delivers exactly the empty page the numeral promised, with an explanation. **CLOSED with no code change — working as designed.** Sam delegated the ratification to the 2026-08-08 overnight session and the recommendation was adopted as decided. The 1.8 lift is honest for `ships_only` only because Criterion 1.7 clears `ships_only` on segment entry; no rule clears the offered-item filters, so `segmentPatch` carries them across the switch and the clicked segment really serves the empty page. Lifting them in the count (together or per-family) would advertise contracts the click cannot deliver — the numeral-wearing silent-no-op 1.8 exists to prevent, and the case `_segment_counts_and_total`'s docstring warns about. Read literally, Criterion 1.8 is scoped to `ships_only` and is fully implemented; `SegmentTabs.tsx`'s count comment already records the served zero as honest. The coherent alternative — also clearing offered-item filters on item-less segment entry, then lifting — is a spec amendment only Sam can ratify. Full reasoning: [`2026-08-08-overnight-followups-decision-log.md`](2026-08-08-overnight-followups-decision-log.md) OD4.
- **Two pre-existing sorts are nullable and place NULL inconsistently with the rule Task B8 establishes.** `SortableContractFields.volume` maps to `Contract.volume` (`models/contracts.py`, `nullable=True`) and `ship_name` maps to `ContractItem.type_name`; neither carries `nulls_last()`, so on PostgreSQL a descending sort by volume leads with every volume-less contract and a descending sort by ship name leads with every item-less one. Task B8 deliberately did not touch them — the step directs that the existing sorts stay byte-identical, and changing a live sort's plan is outside an additive task — but the frontend now offers three sorts that put "no value" last beside two that put it first, which reads as a bug to anyone who tries both. **CLOSED by the 2026-08-08 overnight follow-up session**: `NULLABLE_SORTS` now covers `volume` and `ship_name`, so all five nullable sorts place NULL last in both directions. Pinned by `test_volume_sorts_both_ways_and_leaves_contracts_without_one_last` and `test_ship_name_sorts_both_ways_and_leaves_item_less_contracts_last` (region 99999972). Decision record: [`2026-08-08-overnight-followups-decision-log.md`](2026-08-08-overnight-followups-decision-log.md) OD3.
...
**Codex round 3, on the readiness rework — one P1 and one P2, both real.**

9. **[P1] The capture was still absorbable mid-request.** For a list key with no cached data yet, React Query reuses the in-flight promise, so a readiness change during that request landed the response carrying the value captured before it, with nothing left to correct it — invalidation cannot help, because it is the same promise. Readiness is now part of the list query key as well, which starts a genuinely new query; `keepPreviousData` then shows the old rows still described by the old answer. `useItemSurfaceRefresh` is deleted as redundant. **The cold-load double fetch that made keying unaffordable in the first place is gone, because `enabled` means the key never moves from unknown to known** — verified as one list request against the live backend.
10. **[P2] The stated five-second bound was false.** With the production `retry: 1`, a hung probe cost two attempts plus the retry delay, ~11s. The taxonomy query now sets `retry: false` — a failed readiness probe just means not-ready, and the five-minute poll comes round again — and the test asserts the bound under a client configured with the production retry default rather than one that hides it.

Both fixes mutation-verified (4 more mutations, 4 killed; 18 in total across the review rounds).

**Codex round 4 — no P1s.** The round-3 findings are closed, deleting `useItemSurfaceRefresh` was safe, and all three rewritten hook tests fail with their fixes reverted (the in-flight one genuinely holds the first request across the readiness change rather than exercising an easier path). One P2 accepted rather than fixed, recorded in Discoveries: a window refocus spanning a readiness change costs one extra corpus-scale list request.

**What the four rounds cost and bought.** Rounds 1–3 each found a P1 *inside the previous round's fix* — 5, then 1, then 1 — before round 4 came back clean. The readiness design ended up needing three mechanisms that are each individually insufficient (`enabled`, capture-in-queryFn, in-the-key), and I arrived at each only by having the previous one shot down. The generalizable part is in decision D13: a rejected alternative can become correct later without anything prompting a re-check. Keying on readiness was rejected in round 0 on a real cost that `enabled` silently eliminated two rounds later. The lesson from D13's reversal is recorded in that entry: "describes the corpus rather than the rows" is not an exemption from WEB-1, because any value the row rendering consumes is about the rows at the moment it is consumed.
