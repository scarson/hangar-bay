...
@pytest.mark.asyncio
async def test_refresh_user_tokens_without_stored_refresh_token_marks_reauth(db_session):
...
async def refresh_user_tokens(db: AsyncSession, http_client: "httpx.AsyncClient", user: User) -> None:
    ...
    rotated) refresh token. Only an invalid-grant-shaped 400 marks the user for
    re-auth (nulls the esi_* columns); 5xx and transport failures re-raise with the
    vault untouched — a transient outage must not force re-login. No M2 endpoint
    calls this — M3's token-using caller does."""
    from ..core import token_cipher
    s = get_settings()
...
**Gates:** Standard TDD gates. Pitfalls: PROXY-1 (routers mount bare — `/auth/sso` prefix, `/me` bare; NO `/api/v1`); §4.4 (shared 503 guard on login+callback, NOT on logout/`/me`); §4.1 callback decision tree (denial → `sso=denied`; GETDEL miss → `sso=error`; GETDEL hit + cookie missing/mismatch → hard 400; all exits clear `hb_sso_state`); §4.2 cookie attributes (HttpOnly, SameSite=Lax, Secure-outside-dev, Path=/, Max-Age); logout idempotent 204; §7 logging — `log_key_event` payloads carry outcome/duration (+ character_id when known) ONLY: **never tokens, JWTs, cipher keys, or state values**.
