...
- **Phase sequencing under the overlay (stacked branches):** do not wait for a merge to start the next phase — the phases are hard-sequential in content, so branch phase N+1 from phase N's branch HEAD. Open phase N+1's PR with **base = phase N's branch** (Phase 0's base is `dev`), and note the stack in the PR body ("Stacked on #<parent PR>"). When Sam merges a parent with `--delete-branch`, GitHub automatically retargets the child PR to the default branch (`dev`) and the diff collapses to just that phase; if the retargeted PR then shows conflicts or replayed commits, rebase the child onto `origin/dev` and `git push --force-with-lease` (mechanics: `docs/git-strategy.md` §Handling merge conflicts).
...
**Gates:** Standard TDD gates adapted (verification = the green CI run driven in Task 0.3). Pitfalls: ENV-1 (backend needs `DATABASE_URL`/`CACHE_URL`/`ESI_USER_AGENT`; `conftest.py` hard-fails without `DATABASE_URL_TESTS`); Python-3.12 pin (locked `pydantic-core` wheels don't support 3.14 — this is why the venv/CI pins 3.12).
...
- [ ] **Step 4 — Run the full backend suite.** `pdm run pytest -q`. Expected: all green. The `test_export_openapi.py` subprocess still passes (export script sets its own env defaults; consolidated Settings keeps every previously-required field). Verify the legacy module is fully gone: `grep -rn "fastapi_app\.config" src` → **zero hits** (the pattern cannot match `fastapi_app.core.config`), and `test ! -f src/fastapi_app/config.py`. Note: `from .config import` inside `src/fastapi_app/core/` resolves to `core.config` and is EXPECTED to remain — do not "fix" those relative imports.
...
async def test_state_is_single_use(auth_client, configured_sso):
    ...
    # the state. (A matching cookie here would reach the code exchange, exit (D), which
    # no test may do without a registered httpx_mock — it would live-POST to EVE.)
...
- [ ] **Step 5 — Commit backend + codegen together (CLAUDE.md).**
