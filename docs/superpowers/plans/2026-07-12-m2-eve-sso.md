...
- **DISC-EXEC-4 (Phase 4, 2026-07-13) — two small deviations from the verbatim Task 4.1/4.2 blocks, both required to clear Standing-Orders gates (pristine test output; no new lint findings).** (1) Task 4.2's `test_hs256_token_rejected` signs a token with a short literal secret (`"shared-secret"`, 13 bytes) purely to build an HS256 token for `validate_access_token` to reject — PyJWT itself warns on this via `jwt.warnings.InsecureKeyLengthWarning` (RFC 7518 §3.2 minimum-key-length advisory), which broke the "EMPTY warnings summary" gate even though the test and `sso.py` both behaved correctly. The warning is about the literal test string's length, not about anything `sso.py` does — HS256 is never used to sign anything in production. Resolved by adding a scoped `filterwarnings = ["ignore::jwt.warnings.InsecureKeyLengthWarning"]` entry to `[tool.pytest.ini_options]` in `app/backend/pyproject.toml`, landed in the same commit as `sso.py`/`test_sso.py`, with an inline comment explaining why. (2) The verbatim `sso.py` Protocol stub `def get_signing_key_from_jwt(self, token: str): ...` (single line) trips `E704 multiple statements on one line (def)` — this repo's `.flake8` uses `select = C,E,F,W,B,B950` with E704 not in `ignore`, and (unlike the many pre-existing findings in older files) a brand-new file introducing a real, selected finding fails the "no NEW findings in touched files" gate. Resolved by moving the `...` body to its own line (`def get_signing_key_from_jwt(self, token: str):` / `        ...`) — identical Protocol-stub semantics, zero lint findings, verified against a stashed pre-change baseline (`pdm run lint` output byte-identical to baseline after the fix). No other content of the Task 4.1/4.2 blocks changed.
