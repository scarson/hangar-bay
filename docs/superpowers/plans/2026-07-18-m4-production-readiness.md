...
| 2 — Platform provisioning | 🚧 2b EXECUTED VIA API 2026-07-21 (D-12) — resources live, domain live w/ cert, `/ready` green on the released SHA | — | Remaining: Sam's `ESI_CLIENT_ID`/`ESI_CLIENT_SECRET` dashboard paste + agent's `TOKEN_CIPHER_KEYS` + one redeploy (atomic SSO enable). `:443` portal removal confirmed by Sam 2026-07-21 (D-3 closed) |
