...
**Overall:** Production is LIVE at `https://hangarbay.app` (ohio, Postgres 18) with CD proven end-to-end: releases #66 (2026-07-19) and #74 (2026-07-21) published; 2b executed via API (D-12/D-13); Phase 4 Steps 0-2 and 4 complete with evidence (pipeline proof, verification sweep, rollback drill); the Jul 23-26 scheduler incident diagnosed and fixed (D-15, PRs #76/#77). **The single remaining M4 item is Phase 4 Step 3: Sam's live SSO login on the production origin** (blocked on EVE character creation — free, needs the game client). Release [#79](https://github.com/scarson/hangar-bay/pull/79) (2026-07-26) deployed the D-15 fixes; its pipeline run 30215356899 was the FIRST fully-green CD run (deploy + smoke) — the deploy pipeline is proven end to end. The region-filter viewport-sizing fix (PR [#80](https://github.com/scarson/hangar-bay/pull/80)) is on dev awaiting the next publication PR. **Ingestion recovery is VERIFIED** — the first completed post-fix run landed 2026-07-26T20:03:21Z, ~77 min after the release deploy (D-15); before that run completed, `/ready` still read 88.5 h stale, which is correct-but-misleading. D-15's flagged-not-fixed residual, the watchlist matcher lock TTL, is PR [#82](https://github.com/scarson/hangar-bay/pull/82).
...
| 2 — Platform provisioning | ✅ COMPLETE 2026-07-21 (D-12, D-13) — all resources live in ohio, domain live w/ cert, SSO enabled and verified (302 → EVE) | — | Executed via API instead of the blueprint (D-12); region moved frankfurt→ohio mid-provisioning (D-13); `:443` portal removal confirmed (D-3 closed) |
...
- **D-3 (prod EVE callback carries `:443`).** Sam registered the prod EVE application with callback `https://hangarbay.app:443/api/v1/auth/sso/callback` (explicit `:443`). At Phase 2b, `ESI_SSO_CALLBACK_URL` must match char-for-char — either set the env var WITH `:443` or Sam first edits the EVE portal to drop it. **RESOLVED 2026-07-19: Sam removes `:443` in the EVE portal**, so `ESI_SSO_CALLBACK_URL` is the clean `https://hangarbay.app/api/v1/auth/sso/callback`; verify the portal edit landed before entering the value at 2b.
- **D-4 (Task 1.1: Dockerfile gained a compile stage).** The plan's two-stage Dockerfile fails to build: the locked `asyncpg 0.30.0`/`httptools 0.6.4`/`uvloop 0.21.0` publish no CPython 3.14 wheels, and `python:3.14-slim` has no compiler (`pip install` dies on missing `assert.h`, then uvloop's bundled-libuv `./configure` needs `make`). Fix stayed inside the allowed file surface: a dedicated `build` stage installs `build-essential` and `pip install --prefix=/install`s the deps; the runtime stage `COPY --from=build /install /usr/local` and stays slim. Verified: image builds, boots prod-shaped, `/health` ok, fail-closed skip line present.
- **D-5 (Task 1.2: `off` enum values quoted).** The plan's inline render.yaml writes `autoDeployTrigger: off` / `generation: off` unquoted; YAML 1.1 parses bare `off` as boolean `false` (verified with pyyaml), which would corrupt these string enums — worst case `autoDeployTrigger` silently reverting to deploy-on-commit, racing CI. All three occurrences are quoted `"off"` in the committed file, matching Render's own spec example (`autoDeployTrigger: 'off'`). Details: `docs/audits/m4-recon/render-docs-verification-2026-07-18.md` §Corrections.
...
- **D-12 (Phase 2b executed agent-side via the Render REST API, 2026-07-21 — Sam-directed).** Sam redirected 2b from the dashboard blueprint flow to API execution ("unclear why these are steps I'm supposed to be doing manually"). The blueprint was therefore never applied; `render.yaml` is documentation-of-record, and future infra changes must be applied via API/dashboard AND mirrored in the yaml. What ran: Key Value `red-d9fi1ef7f7vs73b7k230` (free, frankfurt, allkeys-lru, `ipAllowList: []`) and Postgres `dpg-d9fi1abrjlhs73aco33g-a` (basic-256mb, PG 17, frankfurt); web service `hangar-bay-api` `srv-d9fif6jtqb8s73d5s8e0` (starter, docker, `/ready` health check, 1 GB scheduler-pin disk) with `fromDatabase`/`fromService` emulated by injecting the stores' internal connection strings via the env-var API (values never printed); static site `hangar-bay-web` `srv-d9fi26favr4c73ca2qa0` with the production rewrites/headers; custom domains `hangarbay.app` + auto-added `www` redirect, Cloudflare **DNS-only** CNAMEs (agent decision: Render's static edge is already Cloudflare-fronted — proxying would stack a second uncacheable-purge layer and complicate cert issuance for no benefit). Assigned hostname = `hangar-bay-api.onrender.com` exactly (render.yaml's destination needed no fix). Actions secrets: `RENDER_API_KEY` set agent-side via stdin (Sam-directed; value untouched by chat/argv), `RENDER_API_SERVICE_ID`/`RENDER_STATIC_SERVICE_ID` set from the created IDs. **Two Render API landmines found (cost three service recreates):** (1) `preDeployCommand` is create-time-only — PATCH silently no-ops (responses echo stored state, which briefly masqueraded as success); (2) the pre-deploy executor mis-tokenizes quoted strings AND `-c` flags (quoted `sh -c "…"` exec'd as one word → exit 127; `python -m alembic -c <ini> upgrade head` reached alembic with the trailing subcommand dropped → "too few arguments"). Working form: `python -m alembic upgrade head` with `ALEMBIC_CONFIG=/app/src/alembic.ini` as an env var — verified live (deploy `live`; `/api/v1/contracts/` returns a real query result through the production rewrite, proving the schema exists). SSO bootstrap is deliberately trio-empty (boots in SSO-disabled state per Task 3.7's fail-fast matrix); Sam pastes `ESI_CLIENT_ID`/`ESI_CLIENT_SECRET` in the dashboard, the agent adds fresh `TOKEN_CIPHER_KEYS` via API in the same step, one redeploy turns SSO on atomically.

- **D-13 (Region moved frankfurt → ohio; SSO enabled, 2026-07-21 — Sam-directed).** Sam chose US interactive latency over ESI (Europe) ingest proximity; the move ran while the DB held only re-ingestable data (zero-loss window). Mechanics: deleted the frankfurt api service + both stores, recreated all three in ohio (`hangar-bay-api` `srv-d9fippfavr4c73cbi1d0`, Postgres `dpg-d9fipb3bc2fs73bam7ag-a`, Key Value `red-d9fipabrjlhs73ae5grg`); the region-less static site (domain + cert) was untouched; the `hangar-bay-api.onrender.com` hostname was re-granted, so the rewrite destination needed no change; `RENDER_API_SERVICE_ID` secret updated; render.yaml regions updated in this PR. Accepted consequence: ESI fetches gain ~80 ms RTT (background work). SSO enablement completed the same session: Sam pasted `ESI_CLIENT_ID`/`ESI_CLIENT_SECRET` ("Save only"), agent added fresh `TOKEN_CIPHER_KEYS` via API before any boot could snapshot a partial trio, then one pinned redeploy. Verification sweep on `https://hangarbay.app`: `/ready` db+cache ok with `commit` = released SHA `f60515c`; `/api/v1/contracts/` 200 through the rewrite; `/api/v1/auth/sso/login` 302 → `login.eveonline.com` (production fail-fast passed with the full trio); `/metrics` 401 without bearer; HSTS rule serving. Remaining Phase 4: pipeline proof (workflow_dispatch), rollback drill, Sam's live SSO login (exit criterion).
- **D-14 (Production Postgres recreated at 18; version pins unified, 2026-07-21 — Sam-directed).** Executed on the assessment's recreate-now verdict (`docs/audits/m4-recon/postgres-18-upgrade-assessment.md` — PG18 upgrade assessment: no breaking change touches this stack; the zero-loss window exists only while the DB holds re-ingestable data): deleted the hours-old PG17 instance, created `hangar-bay-db` at PG 18 (`dpg-d9fj14btqb8s73d71r10-a`, ohio, basic-256mb), rewired `DATABASE_URL`, one pinned deploy laid the schema via the Alembic pre-deploy. Verified live: `/ready` db ok, `/api/v1/contracts/` 200, SSO 302, instance reports version 18. Same PR unifies the version pins the assessment flagged (CI service container and dev compose were still on 16 — never matched prod's 17): `render.yaml postgresMajorVersion: "18"`, `ci.yml postgres:18`, `compose.dependencies.yml postgres:18-alpine`. **Local-dev consequence:** an existing `postgres_db` compose volume holds 16-format data and postgres 18 will refuse to start on it — drop the volume (dev data is disposable, ENV-2) and let 18 initialize fresh. asyncpg 0.31+ bump (official PG18 support, cp314 wheels) runs as a parallel PR on `claude/asyncpg-bump`.

- **D-15 (Production incident: scheduler silently dead Jul 23-26; diagnosed and fixed 2026-07-26).** Last scheduler tick 2026-07-23T01:34:45Z; the 01:32 aggregation run (70 min, 155k items) drove the free `allkeys-lru` Key Value instance into LRU eviction, which ate APScheduler's RedisJobStore keys — no due jobs, no errors, 3.6 days of silence while the site served stale data (`/ready`'s `data_stale: true` was the only honest signal, and it was correct). Same logs surfaced a second latent bug: the run outlived the fixed 1800 s aggregation lock TTL ("lock token mismatch on release"). Fixes: PR [#76](https://github.com/scarson/hangar-bay/pull/76) (MemoryJobStore — boot re-registers jobs, Redis persistence bought nothing; lock TTL now derives from the scheduler interval + 300 s margin; pitfall DEPLOY-3; flagged-not-fixed: watchlist lock TTL equals its 900 s interval, Sam's call) and PR [#77](https://github.com/scarson/hangar-bay/pull/77) (the CD smoke's pagination spec sampled page-1 rows kept rendered by `keepPreviousData` as placeholder before the page-2 fetch resolved — deterministic synchronization added per TEST-2; verified 3/3 against live production). Interim remediation: the Phase 4 rollback drill's restarts re-registered the jobs — but **no aggregation run actually COMPLETED until 2026-07-26T20:03:21Z**, because each restart aborted the run its predecessor had started (the job carries `next_run_time=now`, so every boot starts a run, and a full uncapped run takes ~77 min — longer than the gaps between the drill's rollback (~18:05), roll-forward (~18:15) and release-#79 (18:46) restarts). **Verified recovery, 2026-07-26T20:03:21Z** — ~77 min after the release-#79 deploy completed at 18:46:44Z: `/ready` flipped to `data_stale: false` (age 29 s, `outcome: success`), contracts rose 40,678 → 50,989, newest `date_issued` advanced Jul 23 01:04 → Jul 26 18:37. This is the first confirmed successful ingest since the incident began; the earlier "revived ingestion" reading was jobs-registered, not data-refreshed. **Operational lesson:** the fix is only observable one full run AFTER the deploy that ships it. Between 18:46 and 20:03, `/ready` correctly reported an 88.5-hour-old last-success (the freshness key advances only on a completed run) — indistinguishable at a glance from an unfixed scheduler, and it prompted a fresh-session agent to open this as a suspected new incident. Judge a scheduler fix by a completed run, not by deploy success. Diagnosis friction worth remembering: the local 1Password `.env` export empties when its approval prompt is dismissed (reads trigger the prompt), which mimicked a revoked API key mid-investigation.

### Discoveries

- **`main` already carries M2 (publication PR [#42](https://github.com/scarson/hangar-bay/pull/42), merged 2026-07-18).** The phases-1+3 handoff's "main is still pre-M2" was stale at writing. The release PR therefore publishes M3 + M4 phases 1+3 + supporting work only — opened as PR [#66](https://github.com/scarson/hangar-bay/pull/66) (2026-07-19, Review — publication, merge held for the spike verdict). Also noted while checking: `scarson/hangar-bay` is a public repo, and the GitHub Actions secret store had NO secrets and no variables as of 2026-07-19 — the agent set repo variable `PROD_ORIGIN=https://hangarbay.app`; the `RENDER_API_KEY` Actions secret remains Sam's to add (2a step 4).
- **Boot-time Valkey outage crashes the process at `scheduler.start()` (pre-existing posture).**

  > **RESOLVED (2026-07-26) as a side effect of D-15's MemoryJobStore change.** This finding's premise was `RedisJobStore` writing pending jobs synchronously at `scheduler.start()`. The jobstore is now in-memory (`core/scheduler.py`), so `scheduler.start()` never touches the cache, and `init_cache` swallows a failed connect (`core/cache.py`: logs CRITICAL, leaves `app.state.redis = None`) instead of raising — a boot-time Valkey outage no longer kills the lifespan, and `/ready`'s cache reinit is now the genuine recovery path rather than a narrow-blip mitigation. **Consequence for M5:** the scheduler-split thread loses this driver and keeps the other one, the single-worker constraint (DEPLOY-2) — scope it on that basis, not on boot-crash tolerance. Original finding preserved below.

  APScheduler's `RedisJobStore` writes pending jobs synchronously when the scheduler starts; with Valkey down at boot the lifespan raises before `yield` and uvicorn exits — recovery is the platform's restart loop, and during a deploy that boot window failing marks the deploy failed. `/ready`'s cache reinit therefore covers only the narrower blip where `init_cache`'s single ping failed but Valkey returned before `scheduler.start()` (codex round-2 finding; docstring corrected to say so). Deferring/tolerating scheduler start on an unavailable cache is a design change parked with the scheduler-split thread (M5).
...
- [x] **Step 1 (pipeline deploy):** re-run `deploy.yml` via `workflow_dispatch` with the released SHA to prove the full pipeline: backend deploy, static deploy, release verification, smoke. *(Proven 2026-07-21/26: the release-#74 `workflow_run` firing's deploy job passed end-to-end — pinned backend deploy incl. Alembic pre-deploy, static deploy, release verification matched `fe4a93ed`; two `workflow_dispatch` firings (the drill) proved the manual path. The smoke job failed in these runs for spec reasons fixed in #75/#77 — see D-15; the deploy pipeline itself never failed after 2b.)*
- [x] **Step 2 (verify):** `curl -s https://<domain>/api/v1/ready | jq .` → `db: ok`, `cache: ok`, `commit` = released SHA; `data_stale: true` initially, flipping false after the first scheduler tick; contracts appear in the SPA. `curl -s -o /dev/null -w '%{http_code}' https://<domain>/metrics` → 401 (token required). *(All verified 2026-07-21 on `https://hangarbay.app`: db/cache ok, commit matched, contracts 200 through the rewrite (40,678 after first ingest), `/metrics` 401, HSTS serving, SSO login 302 → EVE.)*
- [ ] **Step 3 (Sam — the M4 exit criterion, spec §9.3):** on the production origin: login → EVE consent → callback → header shows character name → `/me` 200 → logout → 204/anonymous again; plus one denial path (`cancel` on the EVE consent screen → `?sso=denied`, no session). Report results in-session; the agent records them here with date. *(Partial 2026-07-21: Sam's login reached EVE's character-selection gate — our client id, callback registration, and state handling all accepted by EVE — but his account has no character; EVE is free-to-play, character creation requires the game client. Positive path beyond EVE's gate + denial path remain open.)*
- [x] **Step 4 (rollback drill — do it once while stakes are low):** `workflow_dispatch` deploy.yml with the previous good SHA; confirm `/ready`'s `commit` reverts. Record duration. *(Done 2026-07-26: rollback to `f60515c` dispatched 18:03:36Z, `/ready` commit reverted and verified by ~18:10 (~7 min); roll-forward to `fe4a93ed` dispatched 18:13:50Z, verified ~18:22 (~8 min). Deploy jobs green both directions; the drill's restarts doubled as the D-15 incident remediation.)*
