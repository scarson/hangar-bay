...
**Decision.** Alternative 3 — **but only once paired with the two mechanisms in the amendment below.** As originally shipped it was wrong, and the codex review of PR #145 said so on two counts that were both verified true.

**Amendment (2026-08-08, after codex review — the original reasoning was factually wrong twice).**

The first draft of this entry argued the stale window was "the seconds between the flip and the next list fetch" and that its worst rendering was "byte-identical to the legitimate rendering for every non-blueprint row". Neither held:

1. **Nothing scheduled a refetch of the signal.** `staleTime` marks data stale; it does not schedule anything. With no `refetchInterval`, a tab left open and focused across an `ENRICHMENT_VERSION` resweep would keep reporting the *previous* answer for the whole ~80 minutes it was false — the exact state D1's gate exists to prevent, and the opposite of D1's "degrades on its own". Fixed: `refetchInterval` at five minutes (React Query does not fire it on a backgrounded tab, so it costs one small request per five minutes of actual use).
2. **Nothing refetched the ROWS when readiness flipped.** The list query has its own cache and its own key; a readiness change did not touch it. So a contract known to hold a blueprint copy would show its BPC badge — computed from `is_blueprint_copy`, ingested since M1 — beside empty Runs/ME/TE cells, because its *terms* had not been written when that page was fetched. That is not "an empty cell indistinguishable from a non-blueprint row": the badge is right there saying the row IS a blueprint, which makes the empty cells read as missing data. Fixed: `useItemSurfaceRefresh` invalidates `['contracts','list']` on a change **between two known answers**, so the rows come forward to the state the columns already describe. The `undefined → first answer` case is deliberately excluded — treating it as a change would cost the second corpus-scale list request on every cold load that alternative 1 was rejected for.

With both in place the original claim is finally true: the window is the length of one list refetch, and it closes itself.

**Boundary WEB-1 still governs, and this does not weaken:** the *segment* stays on the response, because it is a function of the request and its stale window renders a sale's price as a hauling reward. Readiness is a different kind of value — a property of the corpus, not of the request — and gets a different treatment. What the review established is that "describes the corpus rather than the rows" is not on its own a licence to read live: it licenses reading live **only when a change in the corpus refreshes the rows**, which is what the amendment adds.

**Reversibility: cheap** — one hook call site each in `FilterRail` and `ContractsPage`, plus the refresh hook; alternative 1 is a two-line change if the coupling ever proves insufficient.
