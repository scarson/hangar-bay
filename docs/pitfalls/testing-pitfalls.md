...
- [ ] **🔥 TEST-14 — A new test added to a module with a file-level `pytest.mark.vcr` replays a cassette and never reaches the app.** vcrpy intercepts *below* `httpx`, ahead of `ASGITransport`, so an API test written in such a module records a cassette on its first run and thereafter asserts against that recording — not against the application, the database, or any code you just changed. It passes identically whether the behavior it names works or is deleted. **Do instead:** put API tests that must exercise real request handling in a module whose `pytestmark` is `asyncio` only (`tests/api/test_contract_filters.py`), and treat an unexpected new file under `tests/api/cassettes/` as the signal that a test is not running what you think. Mutation-verification (TEST-12) catches this and nothing else reliably does — but only if the mutation is run against *that* test, since sibling non-VCR tests in the same run will fail correctly and mask it. **Bit us:** the M5 expiry filter (2026-07-26) added `test_detail_still_serves_an_expired_contract` to the VCR-marked `tests/api/test_contracts.py`; it passed with the expiry predicate deleted, leaving the branch's headline decision — that the detail endpoint deliberately keeps serving expired contracts — with zero effective coverage. The same module also hid eight fixtures with a hardcoded past `date_expired`; those are now anchored to the clock through the module's `LIVE_EXPIRY` constant (TEST-17), but **the masking is structural and still live** — every test in the module still replays. Measured 2026-08-01 by stripping the module's `pytest.mark.vcr` and re-running: the anchored fixtures pass 5/5 against the real app and DB, the pre-fix literals fail 4/5 with `assert 0 == 10`, and with the marker restored those same broken literals pass 5/5 again. The cassettes are committed and still carry `date_expired: "2025-01-02T00:00:00Z"` from a recording made before the expiry filter existed, so the recorded responses encode behavior the app no longer has. Relates to TEST-12 and §7 (test doubles must be honest).
