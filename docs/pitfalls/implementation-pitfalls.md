...
* Before filtering on any ingested column, confirm the route you actually call populates it. The public item route (`/v1/contracts/public/items/{id}/`) carries `is_blueprint_copy`, `runs`, `material_efficiency`, `time_efficiency`. The authenticated character/corporation item routes carry `raw_quantity` and `is_singleton` and **not** those four. `raw_quantity`'s documented `-1`/`-2` blueprint markers therefore do not exist on any data Hangar Bay ingests. Four such columns are kept on purpose — `contracts.status`, `contracts.date_completed`, `contract_items.is_singleton`, `contract_items.raw_quantity` — because ESI's authenticated character/corporation contract and contract-item routes do populate all four, and ingesting a user's own contracts is a live direction. Retention is cheap (nullable columns, no query cost) where a filter or a response field over them is not: **keep the column, and refuse to read it** until the writer that fills it exists.
