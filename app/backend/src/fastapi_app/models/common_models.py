...
class User(Base):
    ...
    username = Column(String, unique=True, index=True, nullable=False)  # E.g., EVE Character Name or local admin username
    email = Column(String, unique=True, index=True, nullable=False) # Must be unique
    hashed_password = Column(String, nullable=True)  # Nullable for EVE SSO users or if not applicable
    is_active = Column(Boolean, default=True)
    eve_character_id = Column(Integer, unique=True, index=True, nullable=True)  # EVE Online Character ID, nullable for non-EVE/admin users
