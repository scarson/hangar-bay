...
## 3. Architectural Implementation: How the Theme is Loaded

It is critical to understand the precise mechanism by which the theme is applied. Misconfiguration here will result in a completely unstyled application.

1.  **`angular.json` is the Entry Point:** The build process starts here. In the `styles` array for both `build` and `test`, it **MUST** point to `src/styles.scss`. Pointing directly to `theme.scss` is incorrect.
2.  **`styles.scss` is the Conductor:** This file acts as the main stylesheet. Its most important job is to import the M3 theme variables from our theme file using `@use './theme';`.
3.  **`theme.scss` is the Source of Truth:** This file is generated by the Angular Material schematic and contains the complete set of M3 system tokens as CSS variables. It should not be modified manually or referenced directly in `angular.json`.

This chain (`angular.json` -> `styles.scss` -> `theme.scss`) is the only correct way to load the M3 theme.

---

## 4. The Core Rules of M3 Theming
...
## 7. Pre-Commit Theming Checklist
...
## 8. Generating & Updating the Theme
