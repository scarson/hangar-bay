...
*   **Logging Management:**
    *   **Chosen (2026-07-19):** Grafana Cloud Loki. The backend's structlog JSON output is duplicated to a file (`LOG_FILE` setting) that a Grafana Alloy container tails and pushes to the managed Loki instance (see `app/backend/docker/compose.observability.yml`).
